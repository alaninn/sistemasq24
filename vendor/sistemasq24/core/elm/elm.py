...
from sistemasq24.core.elm.constants import (
    cmdb,
    negrsp
)
from sistemasq24.core.elm.device_manager import DeviceManager
from sistemasq24.core.elm.port import Port
from sistemasq24.core.usbdevice.usb_can import UsbCan
from sistemasq24.file_manager import get_logs_dir
            ...
            if line.startswith("7F") and len(line) == 8 and line[6:8] in negrsp.keys():
                ...
                if self.vf != 0:
                    ...
                    if self.currentaddress in list(dnat.keys()):
                        tmp_addr = dnat.get(self.currentaddress, self.currentaddress)
                    tmstr = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        ...
        if noerrors and len(result) // 2 >= nBytes:
            ...
            if result[:2] == '7F' and result[4:6] in list(negrsp.keys()):
                if self.vf != 0:
                    tmstr = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    self.vf.write(
                        tmstr + ";" + dnat.get(self.currentaddress, self.currentaddress) + ";" + command + ";" + result + ";" + negrsp[
                            result[4:6]] + "\n")
                    self.vf.flush()
